 ...
 * 
 * @param entries - Array of file entries with path and desired name in the ZIP
 * @param outputPath - Path for the output ZIP file
 * @returns Path to the created ZIP file
 */
export async function createZipFromFiles(
  entries: string[] | { filePath: string; nameInZip: string }[],
  outputPath: string
): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    for (const entry of entries) {
      if (typeof entry === "string") {
        // Legacy: just a file path
        if (!fs.existsSync(entry)) {
          console.warn(`[ZIP] File not found, skipping: ${entry}`);
          continue;
        }
        const fileName = path.basename(entry);
        archive.file(entry, { name: fileName });
      } else {
        // New: file path + custom name in ZIP
        if (!fs.existsSync(entry.filePath)) {
          console.warn(`[ZIP] File not found, skipping: ${entry.filePath}`);
          continue;
        }
